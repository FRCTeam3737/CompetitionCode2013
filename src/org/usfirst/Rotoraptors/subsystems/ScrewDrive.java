...
public class ScrewDrive extends Subsystem {
    ...
    public void control(double input) {
        // If running forwards (opposite, I know) and encoder reads past cutoff point, stop drive
        if((input < 0) && (getShooterDist() > Constants.Shooter.MAX_SHOOTER_DIST)) {
            screwCIM.set(0);
        // Else if getting close to the limit switch according to encoder, and running
        // backward (opposite...), go half speed
        } else if ((getShooterDist() < 1) && (input > 0) && (!getScrewLimit())) {
            screwCIM.set(input / 2);
        // If the lift ever hits the limit switch, stop all further movement and reset lift encoder
        } if ((input >= 0) && getScrewLimit()) {
            screwCIM.set(0);
            resetLiftEncoder();
        // Otherwise act normally, set speed to joystick value
        } else {
            screwCIM.set(input);
        }
    }
    ...
    public void setShooterDist(double dist) {
        // Where distance = the direct encoder value as opposed to an angle
    ...
    // These equations need to be reworked/refigured entirely
